...
class Product():
    ...
    async def get_info(self, product_id, catalog_id, subdirectory_id):
        conn, cursor = connect()

        cursor.execute(f'SELECT * FROM {catalog_id if subdirectory_id is None else subdirectory_id} WHERE product_id = "{product_id}"')
    ...
    async def get_all_products_in_subdirectory(self, subdirectory_id):
        conn, cursor = connect()

        cursor.execute(f'SELECT * FROM {subdirectory_id}')
        self.products = cursor.fetchall()

    async def get_all_subdirectory_in_catalog(self, catalog_id):
        conn, cursor = connect()

        cursor.execute(f'SELECT * FROM subdirectories WHERE catalog_id = "{catalog_id}"')
        self.subdirectories = cursor.fetchall()

    async def get_menu_del_product_subdirectories(self, subdirectory_id):
        await self.get_all_products_in_subdirectory(subdirectory_id)

        markup = types.InlineKeyboardMarkup(row_width=2)

        for i in self.products:
            markup.add(types.InlineKeyboardButton(text=f'{i[1]}',
                                                  callback_data=f'del_product:{i[0]}:{subdirectory_id}'))

        return markup

    async def get_menu_products(self, catalog_id, subdirectory_id=None, type_directory=None):
        markup = types.InlineKeyboardMarkup(row_width=2)

        if type_directory is None:
            await self.get_all_products_in_catalog(catalog_id)
            await self.get_all_subdirectory_in_catalog(catalog_id)

            for i in self.subdirectories:
                markup.add(types.InlineKeyboardButton(text=f'{i[2]}', callback_data=f'subdirectory:{i[1]}:{catalog_id}'))
        else:
            await self.get_all_products_in_subdirectory(subdirectory_id)
        ...
        for i in self.products:
            markup.add(
                types.InlineKeyboardButton(
                    text=f'{i[1]} ⋆ {i[2]} руб', callback_data=f'product:{i[0]}:{catalog_id}:{subdirectory_id}' if type_directory is not None else f'product:{i[0]}:{catalog_id}'))

        if type_directory is None:
            markup.add(types.InlineKeyboardButton(text=f'« Назад', callback_data=f'back_to_catalog'))
        else:
            markup.add(types.InlineKeyboardButton(text=f'« Назад', callback_data=f'catalog:{catalog_id}'))
        return markup

    async def get_preview_text(self, product_id, catalog_id, subdirectory_id=None):
        await self.get_info(product_id, catalog_id, subdirectory_id)
    ...
    async def get_preview_menu(self, product_id, catalog_id, subdirectory_id=None):
        markup = types.InlineKeyboardMarkup(row_width=1)

        markup.add(
            types.InlineKeyboardButton(
                text=f'Купить',
                callback_data=f'preview_buy_menu:{product_id}:{catalog_id}' if subdirectory_id is None else f'preview_buy_menu:{product_id}:{subdirectory_id}'),
            types.InlineKeyboardButton(
                text=f'« Назад',
                callback_data=f'catalog:{catalog_id}' if subdirectory_id is None else f'subdirectory:{subdirectory_id}:{catalog_id}')
        )
    ...
    async def get_buy_menu(self, product_id, catalog_id, amount=None, price=None, update=None):
        ...
        if amount == None and price == None:
            await self.get_info(product_id, catalog_id, None)
    ...
    async def create_product(self, catalog_id, name, description, price):
        ...
        product_id = f'p{randint(0, 999999)}'
    ...
    async def get_menu_del_product(self, catalog_id, type_directory=None):
        if type_directory is None:
            await self.get_all_products_in_catalog(catalog_id)
        elif type_directory == 'subdirectory':
            await self.get_all_products_in_subdirectory(catalog_id)

        markup = types.InlineKeyboardMarkup(row_width=2)

        for i in self.products:
            markup.add(types.InlineKeyboardButton(text=f'{i[1]}',
                                                  callback_data=f'del_product:{i[0]}:{catalog_id}'))
    ...
    async def get_menu_upload_product(self, catalog_id=None, subdirectory_id=None):
        if subdirectory_id is None:
            await self.get_all_products_in_catalog(catalog_id)
        else:
            await self.get_all_products_in_subdirectory(subdirectory_id)
        ...
        for i in self.products:
            markup.add(types.InlineKeyboardButton(text=f'{i[1]}',
                                                  callback_data=f'upload_product:{i[0]}:{catalog_id}' if subdirectory_id is None else f'upload_product:{i[0]}:{subdirectory_id}'))

        return markup

    async def get_menu_upload_subdirectory(self, catalog_id):
        await self.get_all_subdirectory_in_catalog(catalog_id)

        markup = types.InlineKeyboardMarkup(row_width=2)

        for i in self.subdirectories:
            markup.add(types.InlineKeyboardButton(text=f'{i[2]}', callback_data=f'get_menu_upload_subdirectory:{i[1]}:{catalog_id}'))
        ...
        try:
            with open(file_name, 'rb') as txt:
                ...
                for i in txt.readlines():
                    ...
                    try:
                        cursor.execute(sql, (i, randint(0, 999999999)))
                        conn.commit()
